...
- Debug 执行：允许在受控环境下运行测试/验证命令（默认主机 `subprocess` 硬化；可选 `docker` 后端已落地为本地 `docker run` 执行路径，见 [execute_tools_design.md](execute_tools_design.md)），保留超时与目录限制。
- 许可证：MIT。
...
- execute 类工具的设计与安全规范见 [execute_tools_design.md](execute_tools_design.md)；Docker 后端、容器跑测和 MVP+ gate 口径见 [mvp_plus_roadmap.md](mvp_plus_roadmap.md)。
