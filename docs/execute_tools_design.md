...
本文档记录项目中 **execute 类工具**（`run_command`、`run_tests`）的设计原则、分层安全策略与配置约定。实现细节以代码为准。契约与环境变量见 [shared_contracts.md](./shared_contracts.md) §2 / §6；编排高危门控见 [cli_tools_orchestrator_contract.md](./cli_tools_orchestrator_contract.md) §11；后续事项见 [维护待办](./maintenance_backlog.md)。
